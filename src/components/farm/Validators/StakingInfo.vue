...
export default {
  ...
  computed: {
    ...mapGetters("addressConstants", ["hMULTICALL", "hRPC", "DONECHEF"]),
  },
  data() {
    return {
      loaded: false,
  ...
  methods: {
    ...
    CALL.push({
      ...
      this.validators[n].earnedDone = (results[3].value.toString() / 10**18).toFixed(2)
      ...
      if(n == 0){
        ...
        this.validators[n].earnedDone = "NA"
