...
export default {
  ...
  methods: {
    ...
    ...mapGetters('farm/farmData', ['getStateDONEPrice', 'getStateOnePrice']),
    ...mapActions('farm/farmData', ['setDONEPrice', 'setOnePrice']),
      ...
      if (address != "0x0000000000000000000000000000000000000011") {
        ...
        for (n in Pools) {
          const pending = await contract
            .pendingDefione(parseInt(Pools[n].pid), address).catch(error => {
              console.log(error);
              this.error = 1;
    ...
    getLiquidityValueSolo: async function(pool, staked){
      ...
      let donePrice = this.getStateDONEPrice();
    ...
    approveV1: async function(token){
      await this.approveSpending(token, this.DEFIONEBRIDGE(this.getChainID()));
    },
    migrateV2: async function(amountIn){
      let amount = this.getUnitsD(amountIn, 18)
      let isDefaultWallet = this.checkSignedIn();
      const abi = DefioneBridge.abi;
      if (isDefaultWallet){
        toastMe('error', {
          title: 'ERROR',
          msg: `Not Signed In`,
      ...
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner();
      const contract = new ethers.Contract(this.DEFIONEBRIDGE(this.getChainID()), abi, signer);
      ...
      if(this.getWalletType() == 'oneWallet'){
        let options = { gasPrice: "0x3B9ACA00" };
        const unattachedContract = hmy.contracts.createContract(abi, this.DEFIONEBRIDGE(this.getChainID()));
        let wallet = new oneWallet()
        await wallet.signin()
        let contract = wallet.attachToContract(unattachedContract)
        //const gas = await contract.methods.withdraw(pid, '0').estimateGas(options).catch();
    ...
    setRatioValidator: async function(amount){
      ...
      const tx = await contract.setRatioPercentageDefione(100 - amount).catch(err => {
          var message;
          if(!err.data?.message){
            message = err.message
          }else{
      ...
      if(this.getWalletType() == 'oneWallet'){
        ...
        var tx1 = await contract.methods.setRatioPercentageDefione((100 - amount).toString()).send(options)
        if(tx1.transaction.txStatus == 'CONFIRMED'){
          this.setBtnState({add: 'added'})
          let transaction = tx1.transaction.id
          this.setBtnState({remove: 'removed'})
