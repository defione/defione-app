<template>
  <div id="tokens" class="max-w-screen-xl mx-auto flex flex-1 flex-col items-center justify-center done-layout xl:px-0 px-3 text-gray-500 pb-16">
        ...
        <div class="flex flex-col flex-1 divide-y divide-black divide-opacity-10 w-full">
          <div class="flex flex-wrap text-gray-500 dark:text-gray-300" v-for="(network, index) in this.retrieveTokens(search, 1666600000)" :key="index">
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
              <!-- icons table -->
              <div v-for="(token, idx) in network.tokens[1666600000]" :key="idx" class="flex flex-col p-3 st5 st5-all group bg-gradient-to-l dark:from-slightDark from-slightGray to-transparent dark:hover:bg-slightDark hover:bg-slightGray border-l border-doneGreen rounded-3xl">
                <div class="w-full flex flex-wrap">
                  <div class="w-full relative flex gap-5 h-12 items-center">
                    ...
                    <p class="text-xs text-doneBlue-light">
                      {{ token.name }}
                  ...
                  <div class="flex w-8/12 flex-col h-full mt-1 relative" v-if="token.oneAddress">
                    <div class="flex gap-5 mb-2 w-6/12">
                      <p class="text-xs flex-none dark:text-doneGreen-dark">Harmony One Address</p>
                      <p class="text-xs flex gap-1 dark:text-gray-300">
                        {{ shortenAddress(token.oneAddress) }}
                        <tooltip-me>
                          <svg @click="copyAddress(token.oneAddress)" class="h-4 w-4 dark:text-doneGreen-dark hover:opacity-50" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                          ...
                          <tooltip-me-content :options="tooltip" class="flex ss:w-64 xs:w-80 rounded-lg shadow-xl p-0.5">
                            <div class="flex w-full space-x-2 items-start bg-gray-100 dark:bg-slightDark text-gray-500 dark:text-gray-300 rounded-md p-3">
                              <svg class="h-6 w-6 dark:text-doneGreen-dark" width="36" height="36" viewBox="0 0 36 36" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" />
                                <rect x="8" y="8" width="12" height="12" rx="2" />
                                <path d="M16 8v-2a2 2 0 0 0 -2 -2h-8a2 2 0 0 0 -2 2v8a2 2 0 0 0 2 2h2" />
                              </svg>
                    ...
                    <div class="flex gap-5 mb-2 w-6/12">
                      <p class="text-xs flex-none dark:text-doneGreen-dark">Harmony 0x Address</p>
                      <p class="text-xs flex gap-1 dark:text-gray-300">
                        {{ shortenAddress(token.oneZeroxAddress) }}
                        <tooltip-me>
                          <svg @click="copyAddress(token.oneZeroxAddress)" class="h-4 w-4 dark:text-doneGreen-dark hover:opacity-50" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" />
                            <rect x="8" y="8" width="12" height="12" rx="2" />
                            <path d="M16 8v-2a2 2 0 0 0 -2 -2h-8a2 2 0 0 0 -2 2v8a2 2 0 0 0 2 2h2" />
                          </svg>
                    ...
                    <div class="flex gap-5 mt-1 mb-2 w-6/12" v-if="token.bscAddress">
                      <p class="text-xs flex-none dark:text-doneGreen-dark">Bsc Address</p>
                      <p class="text-xs flex gap-1 dark:text-gray-300">
                        {{ shortenAddress(token.bscAddress) }}
                        <tooltip-me>
                          <svg @click="copyAddress(token.bscAddress)" class="h-4 w-4 dark:text-doneGreen-dark hover:opacity-50" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" />
                            <rect x="8" y="8" width="12" height="12" rx="2" />
                            <path d="M16 8v-2a2 2 0 0 0 -2 -2h-8a2 2 0 0 0 -2 2v8a2 2 0 0 0 2 2h2" />
                          </svg>
                    ...
                    <div class="flex gap-5 mt-1 mb-2 w-6/12" v-if="token.ethAddress">
                      <p class="text-xs flex-none dark:text-doneGreen-dark">Eth Address</p>
                      <p class="text-xs flex gap-1 dark:text-gray-300">
                        {{ shortenAddress(token.ethAddress) }}
                        <tooltip-me>
                          <svg @click="copyAddress(token.ethAddress)" class="h-4 w-4 dark:text-doneGreen-dark hover:opacity-50" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" />
                            <rect x="8" y="8" width="12" height="12" rx="2" />
                            <path d="M16 8v-2a2 2 0 0 0 -2 -2h-8a2 2 0 0 0 -2 2v8a2 2 0 0 0 2 2h2" />
                          </svg>
                  ...
                  <div class="w-4/12">
                    <button @click="addTokenToMetamask(token)" class="flex items-center justify-center ml-3 rounded-full border border-doneGreen st5 text-doneGreen hover:bg-doneGreen hover:text-gray-200 dark:hover:text-slightDark">
